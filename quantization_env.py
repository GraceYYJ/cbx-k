...
class QuantizationEnv(object):
    ...
    #step by yyj
    def step(self, action, control_bit):
        #如果得到的b是全0的，则需要重新随机一个b作为action
        self.action_bin[control_bit] = mc.binazation(action)
        #self.action_b[control_bit]= action.numpy()
        #action_b=self.b
        ...
        if self.is_first_v:
            ...
            #self.soft_c_v = self.compute_v(self.compute_Cbx(self.action_b))
        ...
        #return self.observation, self.reward, self.mask, self.c_v
